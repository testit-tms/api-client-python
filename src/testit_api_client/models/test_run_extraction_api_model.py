...
class TestRunExtractionApiModel(BaseModel):
    """
    Rules for different level entities inclusion/exclusion  # noqa: E501
    """
    ids: Optional[GuidExtractionModel] = Field(default=None, description="Extraction parameters for test runs")
    __properties = ["ids"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> TestRunExtractionApiModel:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> TestRunExtractionApiModel:
        """Create an instance of TestRunExtractionApiModel from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return TestRunExtractionApiModel.parse_obj(obj)

        _obj = TestRunExtractionApiModel.parse_obj({
